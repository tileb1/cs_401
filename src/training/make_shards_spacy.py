...
BASE_PROCESSED = "datasets/img2dataset/laion400m-data-processed"
BASE_RAW = "datasets/img2dataset/laion400m-data"
MAX_SHARD_ID = 20703
...
def extract_nouns(tokenizer, text):
    ...
    tokenizer_decoder = tokenizer.decoder
    ...
    tensor_nouns_indices = torch.zeros(
        [MAX_NOUNS_PER_CAPTION, MAX_TOKENS_PER_NOUN], dtype=torch.long
    )  # nb nouns per caption x nb_tokens per nouns
    tensor_concept_indices = torch.zeros(
        [MAX_NOUNS_PER_CAPTION, MAX_TOKENS_PER_NOUN], dtype=torch.long
    )
        ...
        for i, np in enumerate(doc.noun_chunks):
            ...
            try:
                for w in np:
                    ...
                    while True:
                        curr, curr_raw, word_indices = get_metadata_next_word(
                            iterator_tokens_np, curr, curr_raw, word_indices
                        )
                        if "".join(curr) == str(w):
            ...
            for w in concept_splitted:
                ...
                while True:
                    curr, curr_raw, word_indices = get_metadata_next_word(
                        iterator_tokens_concepts, curr, curr_raw, word_indices
                    )
                    if "".join(curr) == str(w):
            ...
            if (
                concept_index < MAX_NOUNS_PER_CAPTION
                and len(np_token_indices) < MAX_TOKENS_PER_NOUN
                and len(concept_token_indices) < MAX_TOKENS_PER_NOUN
            ):
                ...
                tensor_concept_indices[concept_index, : len(concept_token_indices)] = (
...
def get_shard_loader(ID):
    print(f"{ID:05d}.tar")
    pipeline = [
        wds.SimpleShardList(os.path.join(BASE_RAW, f"{ID:05d}.tar")),
        wds.split_by_worker,
...
def process_shard(ID):
    ...
    with wds.TarWriter(os.path.join(BASE_PROCESSED, f"{ID:05d}.tar")) as sink:
        for i, sample in enumerate(get_shard_loader(ID)):
            tensor_nouns_indices, tensor_concept_indices, all_nouns, all_concepts = (
                extract_nouns(tokenizer, sample["txt"].decode("utf-8"))
            )
