...
class RandomResizedCropWithPos(RandomResizedCrop):
    def __init__(self, *args, **kwargs):
        ...
        max_size = 10000
        x = torch.arange(max_size).repeat(max_size, 1)[None, :]
        RandomResizedCropWithPos._pos = torch.cat(
            (x, x.permute(0, 2, 1)), dim=0
        ).float()
        self.scale = (0.5, 1.0)
    ...
    def forward(self, img):
        ...
        if mask is not None:
            mask = F_viz.resized_crop(
                mask, i, j, h, w, self.size, transforms.InterpolationMode.NEAREST
            )
    ...
    def __call__(self, img):
        ...
        if flip_bool == 1:
            return (
                img,
                F_viz.hflip(pos),
                F_viz.hflip(mask) if mask is not None else None,
            )
        return img, pos, mask
