...
class COCOSegmentationEZ(CocoDetection):
    ...
    def __getitem__(self, index):
        ...
        if len(channel_dict) > 0:
            masks = torch.stack(
                [torch.tensor(channel_mask) for channel_mask in channel_dict.values()]
            )
        ...
        masks = torch.nn.functional.interpolate(
            masks[None, :],
            size=(ht // self.patch_size, wt // self.patch_size),
            mode="nearest",
        ).squeeze(0)
...
class COCOSegmentationEZMapped80(COCOSegmentationEZ):
    def __init__(self, patch_size, transforms, **kwargs):
        ...
        with open(coco_things_classes_path, "r") as f:
            ...
            self.label_to_index = {
                int(k): i for i, k in enumerate(self.id_to_class_coco.keys())
            }
    ...
    def __getitem__(self, index):
        ...
        if labels is not None:
            ...
            onehotmasks = torch.zeros(
                len(self.label_to_index), *masks.shape[1:], dtype=torch.uint8
            )
