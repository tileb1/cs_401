...
class TextTransformer(nn.Module):
    ...
    def build_cls_mask(self, text, cast_dtype: torch.dtype):
        ...
        additive_mask = torch.zeros(
            B,
            c_len + self.embed_cls_nb,
            c_len + self.embed_cls_nb,
            dtype=cast_dtype,
            device=cls_mask.device,
        )
        additive_mask[:, -self.embed_cls_nb :, :c_len][
            cls_mask.unsqueeze(1).repeat(1, self.embed_cls_nb, 1)
        ] = float("-inf")
        additive_mask[:, -self.embed_cls_nb :, -self.embed_cls_nb :][
            torch.zeros(self.embed_cls_nb, self.embed_cls_nb)
            .unsqueeze(0)
            .repeat(B, 1, 1)
            .bool()
        ] = float("-inf")
