...
def _build_vision_tower(
    ...
    if vision_cfg.timm_model_name:
        ...
        # visual = OursDinoVisionTransformer(
        ...
        #     init_values=vision_cfg.init_values,
        #     block_chunks=vision_cfg.block_chunks,
        ...
        # print(visual.load_state_dict(state_dict, strict=False))

        wrapped = ViTWrapper(
            model_type=vision_cfg.timm_model_name_nooverride,
            stride=vision_cfg.patch_size,
        )
        visual = Denoiser(
            vit=wrapped,
            denoiser_type=vision_cfg.denoiser_type,
            image_size=vision_cfg.image_size,
            patch_size=vision_cfg.patch_size,
        )
        if vision_cfg.denoiser_type is not None:
            state_dict = torch.load(vision_cfg.checkpoint_path)
            print(visual.load_state_dict(state_dict["denoiser"], strict=False))
    else:
...
class OursCLIP(CustomTextCLIP):
    ...
    def __init__(self, parsed_args, *args, **kwargs):
        ...
        kwargs["vision_cfg"]["n_decoder_layers"] = parsed_args.n_decoder_layers
        kwargs["vision_cfg"]["n_decoder_heads"] = parsed_args.n_decoder_heads

        # Classification layer
        self.n_classes = parsed_args.n_classes
        # self.beta_ema = parsed_args.beta_ema
        self.register_buffer(
            "classification_weights_ema",
            torch.zeros([self.n_classes + 1, kwargs["embed_dim"]]),
        )
        # self.register_buffer('beta_ema', torch.tensor(self.beta_ema))
        self.register_buffer("beta_ema", torch.tensor(parsed_args.beta_ema))

        # Initialize the decoder
        self.register_buffer(
            "logit_scale_local",
            self.init_logit_scale * torch.ones(1, requires_grad=False),
        )
        if self.args.n_dense_cls > 0:

            if self.args.decoder_type == "TransformerDecoder":
                decoder_layer = CrossModalDecoderLayer(
                    d_model=kwargs["vision_cfg"]["width"],
                    nhead=kwargs["vision_cfg"]["n_decoder_heads"],
                    norm_first=True,
                )
                self.decoder = nn.TransformerDecoder(
                    decoder_layer, num_layers=kwargs["vision_cfg"]["n_decoder_layers"]
                )
            elif self.args.decoder_type == "shallow":
                self.decoder = ShallowDecoder(parsed_args, kwargs["embed_dim"])
            else:
                raise NotImplemented

            # Initialize logits scale/bias for local loss
            # self.logit_scale_local = nn.Parameter(torch.ones([]) * self.init_logit_scale)
            if self.logit_bias is not None:
                self.logit_bias_local = nn.Parameter(
                    torch.ones([]) * self.init_logit_bias
                )
            else:
                self.logit_bias_local = None
        else:
            # self.logit_scale_local = torch.ones(1, requires_grad=False, dtype=torch.float32)
            self.logit_bias_local = None
    ...
    def encode_image(self, image, normalize: bool = False):
        ...
        return (
            {k: F.normalize(v, dim=-1) for k, v in dict_img.items()}
            if normalize
            else dict_img
        )
    ...
    def forward(
        ...
        eos_txt, tokens_txt = (
            self.encode_text(text, normalize=True, return_all=True)
            if text is not None
            else None
        )
            ...
            if n_max_nouns.item() == 0.0 or n_max_tokens.item() == 0.0:
                ...
                tokens_txt = torch.cat(
                    [
                        tokens_txt,
                        torch.zeros(
                            [b, 1, d], dtype=tokens_txt.dtype, device=tokens_txt.device
                        ),
                    ],
                    dim=1,
                )
                ...
                concepts_sequence = torch.gather(
                    input=tokens_txt[:, None].expand(
                        -1, concepts_indices.shape[1], -1, -1
                    ),
                    dim=2,
                ...
                valid_queries = rearrange(
                    concepts_indices.min(dim=-1).values != n, "b n -> (b n)"
                )
                ...
                classification_weights = torch.zeros(
                    [self.n_classes + 1, d],
                    device=concepts_sequence.device,
                ...
                classification_weights = F.normalize(
                    classification_weights, dim=-1, p=2
                )

                # Get the EMA classification weights
                classification_weights_ema = self.classification_weights_ema
                classification_weights_ema = (
                    self.beta_ema * classification_weights_ema.detach()
                    + (1.0 - self.beta_ema) * classification_weights
                )
                classification_weights_ema = F.normalize(
                    classification_weights_ema, dim=-1, p=2
                )
                ...
                concepts_sequence = rearrange(
                    concepts_sequence, "(b n) d -> b n d", b=b
                )
                ...
                cls_embeds_visual = F.normalize(
                    rearrange(cls_embeds_visual, "n b d -> (b n) d"), dim=-1, p=2
                )
                ...
                concepts_labels = rearrange(concepts_labels, "b n -> (b n)")[
                    valid_queries
                ]
        else:
            cls_embeds_txt = None
        ...
        if self.args.image_features_type == "cls":
            ...
            texttopatch = torch.einsum(
                "b d, b n d -> b n", eos_txt, dict_img["x_patchtokens"]
            )
            texttopatch = F.softmax(texttopatch, dim=-1)
            image_features = torch.einsum(
                "b n d, b n -> b d", dict_img["x_patchtokens"], texttopatch
            )
...
class OursDinoVisionTransformer(DinoVisionTransformer):
    def __init__(self, image_size=224, *args, **kwargs):
        super().__init__(*args, **kwargs)
...
class CrossModalDecoderLayer(nn.TransformerDecoderLayer):
    ...
    def forward(
        ...
        if self.norm_first:
            ...
            text = text + self._sa_block(
                self.norm1(text), tgt_mask, tgt_key_padding_mask, tgt_is_causal
            )
            vision = self._mha_block(
                self.norm2(text),
                vision,
                memory_mask,
            ...
            vision = vision + self._ff_block(self.norm3(vision))
        else:
            ...
            text = self.norm1(
                text
                + self._sa_block(text, tgt_mask, tgt_key_padding_mask, tgt_is_causal)
            )
            vision = self.norm2(
                self._mha_block(
                    text, vision, memory_mask, memory_key_padding_mask, memory_is_causal
                )
            )
            vision = self.norm3(vision + self._ff_block(vision))
...
class ShallowDecoder(nn.Module):
    ...
    def forward(self, text, vision):
        texttopatch = torch.einsum(
            ...
            vision
            + self.args.pos_alpha
            * rearrange(self.pe, "b M d -> M b d")[: vision.shape[0]],
        )
        texttopatch = F.softmax(
            texttopatch / self.args.temp_decoder, dim=self.softmax_dim
        )
