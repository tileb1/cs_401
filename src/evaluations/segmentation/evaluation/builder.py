...
def build_seg_dataset(config, untar_path=None, local_rank=0):
    """Build a dataset from config."""
    cfg = mmcv.Config.fromfile(config)

    # Update the untar_path in the config
    if untar_path:
        cfg["data"]["test"]["untar_path"] = untar_path
        cfg["data"]["test"]["local_rank"] = local_rank
...
def build_seg_inference(
    ...
    seg_model = DinoCLIP_Infrencer(
        model,
        num_classes=len(classnames),
        has_background=dset_cfg.has_background,
        **kwargs,
        **config.evaluate
    )
